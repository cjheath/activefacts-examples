...
require 'activefacts/generators/sql/mysql'

ACTUAL_MYSQL_PATH = 'actual/sql/mysql'
FileUtils.mkdir_p(ACTUAL_MYSQL_PATH)

# Generate and return the SQL for the given vocabulary
def generate_mysql(vocabulary)
  output = StringIO.new
  @dumper = ActiveFacts::Generators::SQL::MYSQL.new(vocabulary.constellation)
...
context "CQL Loader" do
  ...
  source_files.each do |source_file|
    expected_file = source_file.sub(%r{cql/(.*).cql\Z}, 'sql/mysql/\1.sql')
    actual_file = source_file.sub(%r{cql/(.*).cql\Z}, ACTUAL_MYSQL_PATH+'/\1.sql')
	...
	actual_text = generate_mysql(vocabulary)
